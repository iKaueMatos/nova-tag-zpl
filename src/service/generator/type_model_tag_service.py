class TypeModelTagService:
    ...
    def generate_code_128(self, zpl: list, sku: str, description: str, label_format: str) -> None:
        if label_format == "1-Coluna":
    ...
    def generate_code_128_full(self, zpl: list, code: str, sku: str, description: str, size: str, label_format: str) -> None:
        zpl.append(f"^LH0,0\n^FO65,18^BY2,,0^BCN,54,N,N^{code}^FS")
        zpl.append(f"^FO145,80^A0N,20,25^FH^{code}^FS")
        zpl.append(f"^FO146,80^A0N,20,25^FH^{code}^FS")
        ...
        zpl.append(f"^FO22,170^A0N,18,18^FH^FDSKU: {sku}")